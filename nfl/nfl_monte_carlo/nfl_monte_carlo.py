...
# Read the data from "weekly_matchups.csv" and "2023_PF.csv" into DataFrames
weekly_matchups = pd.read_csv('nfl/nfl_monte_carlo/nfl_data_scraper/weekly_matchups.csv')
pf_df = pd.read_csv('nfl/nfl_monte_carlo/nfl_data_scraper/points_forced.csv')
pa_df = pd.read_csv('nfl/nfl_monte_carlo/nfl_data_scraper/points_allowed.csv')

# Strip leading and trailing whitespace from team names
weekly_matchups['team_name_1'] = weekly_matchups['team_name_1'].str.strip()
weekly_matchups['team_name_2'] = weekly_matchups['team_name_2'].str.strip()
pf_df['team'] = pf_df['team'].str.strip()
pa_df['team'] = pa_df['team'].str.strip()

# Merge the DataFrames on 'team_name_1' and 'team_name_2'
merged_df = weekly_matchups.merge(pf_df, left_on='team_name_1', right_on='team', how='left')
merged_df = merged_df.rename(columns={'pf_average': 'team_1_pf'})

# Merge again for the second team (team_name_2)
merged_df = merged_df.merge(pf_df, left_on='team_name_2', right_on='team', how='left')
merged_df = merged_df.rename(columns={'pf_average': 'team_2_pf'})

# Merge again for the second team (team_name_2)
merged_df = merged_df.merge(pa_df, left_on='team_name_1', right_on='team', how='left', suffixes=('_team_1', '_team_2'))
merged_df = merged_df.rename(columns={'pa_average_team_1': 'team_1_pa', 'pa_average_team_2': 'team_2_pa'})

# Merge again for the second team (team_name_2)
merged_df = merged_df.merge(pa_df, left_on='team_name_2', right_on='team', how='left', suffixes=('_team_1', '_team_2'))
merged_df = merged_df.rename(columns={'pa_average_team_1': 'team_1_pa', 'pa_average_team_2': 'team_2_pa'})

# Add timestamp fields from points_forced and points_allowed
...
# Specify the output path for the CSV in the current working directory (CWD)
predictions_csv_file_path = 'web_app/nfl_monte_carlo.csv'
log_csv_file_path = 'analytics/nfl_monte_carlo_log.csv'

# Save the DataFrame to a CSV file in CWD
